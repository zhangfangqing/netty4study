...
/**
 * Single-thread singleton {@link EventExecutor}. It starts the thread
 * automatically and stops it when there is no task pending in the task queue
 * for 1 second. Please note it is not scalable to schedule large number of
 * tasks to this executor; use a dedicated executor.
 */
public final class GlobalEventExecutor extends AbstractEventExecutor {
	...
	final ScheduledFutureTask<Void> purgeTask = new ScheduledFutureTask<Void>(this, delayedTaskQueue, Executors.<Void> callable(new PurgeTask(), null), ScheduledFutureTask.deadlineNanos(SCHEDULE_PURGE_INTERVAL), -SCHEDULE_PURGE_INTERVAL);
	...
	public <V> ScheduledFuture<V> schedule(Callable<V> callable, long delay, TimeUnit unit) {
		...
		return schedule(new ScheduledFutureTask<V>(this, delayedTaskQueue, callable, ScheduledFutureTask.deadlineNanos(unit.toNanos(delay))));
	}
	...
	public ScheduledFuture<?> scheduleAtFixedRate(Runnable command, long initialDelay, long period, TimeUnit unit) {
		...
		return schedule(new ScheduledFutureTask<Void>(this, delayedTaskQueue, Executors.<Void> callable(command, null), ScheduledFutureTask.deadlineNanos(unit.toNanos(initialDelay)), unit.toNanos(period)));
	}
	...
	public ScheduledFuture<?> scheduleWithFixedDelay(Runnable command, long initialDelay, long delay, TimeUnit unit) {
		...
		return schedule(new ScheduledFutureTask<Void>(this, delayedTaskQueue, Executors.<Void> callable(command, null), ScheduledFutureTask.deadlineNanos(unit.toNanos(initialDelay)), -unit.toNanos(delay)));
	}
		...
		public void run() {
			for (;;) {
				...
				if (taskQueue.isEmpty() && delayedTaskQueue.size() == 1) {
					synchronized (stateLock) {
						// Terminate if there is no task in the queue (except
						// the purge task).
						if (taskQueue.isEmpty() && delayedTaskQueue.size() == 1) {
