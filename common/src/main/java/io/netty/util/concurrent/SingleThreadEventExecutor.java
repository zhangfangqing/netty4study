...
public abstract class SingleThreadEventExecutor extends AbstractEventExecutor {
	...
	/**
	 * SingleThreadEventExecutor封装的状态： ST_NOT_STARTED ：还没有开始 ST_STARTED：已经开始
	 * ST_SHUTTING_DOWN：正在shutdown ST_SHUTDOWN:已经shutdown ST_TERMINATED:已停止
	 */
	...
	/**
	 * 标示在往任务队列中添加任务时，是否唤醒阻塞在select()上的线程 false: 表示唤醒 true: 标示不唤醒 为什么需要这个变量？
	 * 因为当通过eventloop的execute方法往任务队列添加任务时，就立马该eventloop的线程，该线程会执行
	 * eventloop的run方法，该方法基本的逻辑是: 任务队列如果有任务，就执行selectNow()，这是一个非阻塞方法，
	 * 而任务队列如果没有任务，就执行被包装后的select()方法，该方法是一个阻塞方法
	 * 所以这里在往eventloop中添加任务时，eventloop上的线程可能处于阻塞状态，那么被添加的任务可能会被 延迟执行，所以这里才需要唤醒
	 */
	...
	public void execute(Runnable task) {
		...
		/**
		 * 检查当前runtime的线程对象是否是group从child数组通过next()
		 * 方法取出的eventloop即SingleThreadEventExecutor封装的thread对象
		...
		if (inEventLoop) {
			...
			addTask(task);
			/**
			 * 1.判断当前线程是否已经停止 2.如果已经停止则从任务队列中删除该任务 如果线程已经停止并且删除任务成功，则抛出异常： throw
